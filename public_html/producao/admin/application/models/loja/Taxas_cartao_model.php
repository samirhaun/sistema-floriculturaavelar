...
class Taxas_cartao_model extends CI_Model {
    ...
    /**
     * Identifica o lote bancario de antecipacao de uma receita.
     * O SIPAG agrupa por data de liquidacao, maquininha e bandeira.
     */
    public function lote_da_receita($receita)
    {
        $forma_pgto = (int) $this->campo($receita, 'forma_pgto');
        if (!in_array($forma_pgto, array(3, 4, 5, 6))) {
            return null;
        }
        ...
        if (!$maquininha_id && $taxa_id) {
            $taxa = $this->db->select('maquininha_cartao_id')
        ...
        if (!$data_pagamento) {
            $data_pagamento = $this->campo($receita, 'data_pago');
        }
        if (!$data_pagamento) {
            $data_pagamento = $this->campo($receita, 'data_vencimento');
        }
        ...
        if (!$maquininha_id || !$bandeira || !$data_lote) {
            return null;
        }

        return array(
            'data_lote' => $data_lote,
            // O banco pode creditar no mesmo dia lotes de vendas de dias
            // diferentes. A origem preserva essa separacao sem alterar o
            // dia de liquidacao usado pelo fluxo de caixa.
            'data_origem_lote' => $this->data_origem_lote($receita, $data_lote),
    ...
    {
        $unicos = array();
        foreach ((array) $lotes as $lote) {
            if (!$lote) {
                continue;
            }
            $unicos[$this->chave_lote($lote)] = $lote;
        }

        foreach ($unicos as $lote) {
            $this->recalcular_lote($lote);
        }
    }

    // Uma venda pode compartilhar a liquidacao com vendas de outros pedidos.
    // Reprocessar a data inteira impede que o ultimo salvamento deixe lotes
    // de antecipacao anteriores incompletos.
    public function recalcular_lotes_por_liquidacao($datas)
    {
        ...
        foreach (array_unique(array_filter((array) $datas)) as $data) {
            $receitas = $this->db->select('forma_pgto, data_vencimento, data_pago, data_liquidacao, maquininha_cartao_id, maquininha_taxa_id, bandeira_cartao')
                ->from('contas_receber')
                ->where_in('forma_pgto', array(3, 4, 5, 6))
                ->where('COALESCE(data_liquidacao, data_pago, data_vencimento) = '.$this->db->escape($data), null, false)
                ->get()->result();
            foreach ($receitas as $receita) {
                $lote = $this->lote_da_receita($receita);
                if ($lote) $lotes[] = $lote;
    ...
    /**
     * Mantem as taxas coerentes quando um pedido e cancelado ou reativado.
     * As receitas permanecem para historico, mas pedidos cancelados nao podem
     * contribuir com taxas individuais nem com lotes de antecipacao.
     */
    public function sincronizar_status_pedido($pedido_id)
    {
        ...
        if (in_array((int) $pedido->status_pedido, array(4, 5))) {
            if (!empty($ids)) {
                $this->db->where_in('contas_receber_id', $ids)
                    ->where_in('origem', array('taxa_maquininha', 'taxa_antecipacao'))
                    ->delete('contas_pagar');
                $this->db->where_in('id', $ids)->update('contas_receber', array('taxa_contas_pagar_id' => null));
            }
        } else {
            foreach ($receitas as $receita) {
                $this->garantir_taxa_maquininha($receita, $pedido_id);
            }
        }
    ...
    /**
     * Exclui as receitas/taxas de um pedido e recompõe os lotes afetados.
     * Deve ser chamado dentro da mesma transacao usada para excluir o pedido.
     */
    public function excluir_financeiro_pedido($pedido_id)
    {
        $lotes = $this->lotes_do_pedido($pedido_id);
        $receitas = $this->db->select('id')
            ->from('contas_receber')
            ->where('pedidos_id', $pedido_id)
            ->get()->result();
    ...
    public function normalizar_bandeira($bandeira)
    {
        ...
        if ($normalizada === 'master' || $normalizada === 'master card' || $normalizada === 'mastercard') {
            return 'Mastercard';
        }
        if ($normalizada === 'visa' || $normalizada === 'visa electron') {
            return 'Visa';
        }
        if ($normalizada === 'elo') {
            return 'Elo';
    ...
    private function garantir_taxa_maquininha($receita, $pedido_id)
    {
        ...
        if (!in_array($forma_pgto, array(2, 3, 4, 5, 6))) {
            return;
        }

        $taxa_bandeira = null;
        if (!empty($receita->maquininha_taxa_id)) {
            $taxa_bandeira = $this->db->select('*')->from('maquininhas_cartao_taxas')
                ->where('id', $receita->maquininha_taxa_id)->where('ativo', 1)
                ->where('deleted_at IS NULL', null, false)->get()->row();
        }
        ...
        $maquininha = $this->db->select('*')->from('maquininhas_cartao')->where('id', $maquininha_id)
            ->where('ativo', 1)->where('deleted_at IS NULL', null, false)->get()->row();
        if (!$maquininha) {
            return;
        }

        $campo = $this->campo_taxa_maquininha($forma_pgto);
        $origem_taxa = $taxa_bandeira ?: $maquininha;
        ...
        $existente = null;
        if (!empty($receita->taxa_contas_pagar_id)) {
            $existente = $this->db->select('id')->from('contas_pagar')->where('id', $receita->taxa_contas_pagar_id)
                ->where('origem', 'taxa_maquininha')->get()->row();
        }
        if (!$existente) {
            $existente = $this->db->select('id')->from('contas_pagar')->where('contas_receber_id', $receita->id)
                ->where('origem', 'taxa_maquininha')->get()->row();
        }
        ...
        $conta = array(
            ...
            'data_pago' => ((int) $receita->status === 1) ? $data : null,
            'plano_contas_id' => $maquininha->plano_contas_taxa_id, 'fornecedores_id' => $maquininha->fornecedor_id,
            'forma_pgto' => $forma_pgto, 'contas_receber_id' => $receita->id,
        ...
        if ($existente) {
            $this->db->where('id', $existente->id)->update('contas_pagar', $conta);
            $taxa_id = $existente->id;
        } else {
            $this->db->insert('contas_pagar', $conta);
            $taxa_id = $this->db->insert_id();
        }
    ...
    private function recalcular_lote($lote)
    {
        ...
        if ($chave_legada !== $chave) {
            $this->db->where('chave_agrupamento', $chave_legada)->delete('contas_pagar');
        }
        $maquininha = $this->db->select('*')
            ->from('maquininhas_cartao')
            ->where('id', $lote['maquininha_cartao_id'])
            ->get()->row();

        $taxa_grupo = $this->db->select('taxa_antecipacao')->from('maquininhas_cartao_taxas')
            ->where('maquininha_cartao_id', $lote['maquininha_cartao_id'])
            ->where('grupo_bandeira', $lote['bandeira_cartao'])
            ->where('ativo', 1)->where('deleted_at IS NULL', null, false)->get()->row();
        $percentual_nominal = $taxa_grupo ? (float) $taxa_grupo->taxa_antecipacao : ($maquininha ? (float) $maquininha->taxa_antecipacao : 0);

        $this->db->select('cr.valor, cr.data_vencimento, cr.data_pago, cr.data_liquidacao, COALESCE(tm.valor, 0) AS taxa_maquininha', false);
        ...
        $this->db->where_in('cr.forma_pgto', array(3, 4, 5, 6));
        $this->db->where('cr.status', 1);
        $this->db->where_not_in('p.status_pedido', array(4, 5));
        $this->db->where('cr.maquininha_cartao_id', $lote['maquininha_cartao_id']);
        $this->db->where('cr.bandeira_cartao', $lote['bandeira_cartao']);
        ...
        foreach ($receitas_candidatas as $receita) {
            ...
            if ($data_pagamento !== $lote['data_lote']) {
                continue;
            }
            if ($this->data_origem_lote($receita, $data_pagamento) !== $lote['data_origem_lote']) {
                continue;
            }
            $base_receita = round((float) $receita->valor - (float) $receita->taxa_maquininha, 2);
            $dias_antecipados = $this->dias_antecipados($this->data_base_antecipacao($receita, $data_pagamento), $lote['data_lote']);
            $base += $base_receita;
            // A SIPAG arredonda a antecipacao de cada transacao antes de
            // somar o lote; arredondar apenas o total cria diferencas de 1
            // centavo quando ha mais de uma venda na mesma bandeira.
            $valor_taxa_bruto += round(($base_receita * $percentual_nominal * $dias_antecipados) / 3000, 2);
        }
        $base = round($base, 2);
        $valor_taxa = round($valor_taxa_bruto, 2);
        $percentual_efetivo = $base > 0 ? round(($valor_taxa / $base) * 100, 4) : 0;

        $existente = $this->db->select('id')
            ->from('contas_pagar')
            ->where('chave_agrupamento', $chave)
            ->get()->row();

        if (!$maquininha || $percentual_nominal <= 0 || $base <= 0 || $valor_taxa <= 0) {
            if ($existente) {
                $this->db->where('id', $existente->id)->delete('contas_pagar');
            }
            return;
        }
        ...
        $conta_pagar = array(
            ...
            'data_vencimento' => $lote['data_lote'],
            'valor' => $valor_taxa,
            'status' => 1,
            'data_pago' => $lote['data_lote'],
            'plano_contas_id' => $maquininha->plano_contas_taxa_id,
            'fornecedores_id' => $maquininha->fornecedor_id,
            // A antecipacao e um abatimento do recebimento de credito.
            // O lote pode reunir parcelas diferentes, por isso usa a
            // categoria generica de credito do fluxo de caixa.
            'forma_pgto' => 3,
            'contas_receber_id' => null,
            'maquininha_cartao_id' => $lote['maquininha_cartao_id'],
            'bandeira_cartao' => $lote['bandeira_cartao'],
            'base_calculo' => $base,
            // Guarda a taxa efetiva ponderada do lote; a configuracao mantem
            // a taxa nominal mensal usada no calculo proporcional aos dias.
            'percentual_taxa' => $percentual_efetivo,
            'origem' => 'taxa_antecipacao',
            'chave_agrupamento' => $chave,
        );

        if ($existente) {
            $this->db->where('id', $existente->id)->update('contas_pagar', $conta_pagar);
        } else {
    ...
    {
        if (!$data_venda || !$data_liquidacao) {
            return 0;
        }

        $vencimento_original = new DateTime($data_venda);
        $vencimento_original->modify('+30 days');
        $this->ajustar_para_dia_util($vencimento_original);
        $liquidacao = new DateTime($data_liquidacao);
        if ($vencimento_original <= $liquidacao) {
            return 0;
        }
        return (int) $liquidacao->diff($vencimento_original)->format('%a');
    }

    private function ajustar_para_dia_util(DateTime $data)
    {
        while ((int) $data->format('N') >= 6 || in_array($data->format('Y-m-d'), $this->feriados_bancarios((int) $data->format('Y')))) {
            $data->modify('+1 day');
        }
    }
    ...
    private function feriados_bancarios($ano)
    {
        ...
        foreach (array(-48, -47, -2, 60) as $dias) {
            $data = clone $pascoa;
            $data->modify(($dias >= 0 ? '+' : '').$dias.' days');
    ...
    {
        $a = $ano % 19; $b = (int) floor($ano / 100); $c = $ano % 100;
        $d = (int) floor($b / 4); $e = $b % 4; $f = (int) floor(($b + 8) / 25);
        $g = (int) floor(($b - $f + 1) / 3); $h = (19 * $a + $b - $d - $g + 15) % 30;
        $i = (int) floor($c / 4); $k = $c % 4; $l = (32 + 2 * $e + 2 * $i - $h - $k) % 7;
    ...
    private function data_origem_lote($receita, $data_lote)
    {
        ...
        // Em novos registros data_pago e a data real do pagamento, portanto
        // identifica o lote de origem mesmo quando varios liquidam juntos.
        // Historicos que nao tinham essa informacao continuam agrupados.
    ...
    {
        $data_pago = $this->campo($receita, 'data_pago');
        if ($data_pago && $data_pago !== $data_lote) {
            return $data_pago;
        }
