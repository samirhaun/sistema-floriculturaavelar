...
class Home_l1_c1 extends TEC_Controller {

    public function __construct() {
        parent::__construct();
        $this->load->model('home/l1_c1_model');
    ...
    public function index()
    {
        ...
        $this->montaTela('home/l1-c1/formulario', $data);
    }

    function salvar_l1_c1(){
        if($this->input->post()){
            $this->load->helper('text');
            $dados['posicao_coluna'] = 'l1-c1';
            $dados['link_pagina'] = '';
            ...
            if(!$this->input->post('ck_link')){
                ...
                $config_up_image_interna = array(
                            'upload_path' => FCPATH.'../files/uploads/l1-c1',
                ...
                if(!empty($_FILES['imagem_interna']['name'])){
                    ...
                    if($this->input->post('imagem_l1_c1_interna')){
                        if ($_FILES['imagem_interna']['name'] != $this->input->post('imagem_l1_c1_interna')) {
                            $apagar = FCPATH.'../files/uploads/l1-c1/' . $this->input->post('imagem_l1_c1_interna');
                            @unlink($apagar);
                        }
                    }
                }else{
                    if($this->input->post('imagem_l1_c1_interna')){
                        $file_data['file_name'] = $this->input->post('imagem_l1_c1_interna');
                    }else{
                        $file_data['file_name'] = '';
                    }
                }
            ...
            if(!empty($_FILES['imagem_capa']['name'])){
                ...
                if($this->input->post('imagem_l1_c1_capa')){
                    if ($_FILES['imagem_capa']['name'] != $this->input->post('imagem_l1_c1_capa')) {
                        $apagar = FCPATH.'../files/uploads/l1-c1/' . $this->input->post('imagem_l1_c1_capa');
                        @unlink($apagar);
                    }
                }
            }else{
                if($this->input->post('imagem_l1_c1_capa')){
                    $file_data['file_name'] = $this->input->post('imagem_l1_c1_capa');
                }else{
                    $file_data['file_name'] = '';
                }
            }
            ...
            if($this->l1_c1_model->salvar_l1_c1($dados, $id))
            {
                $_GET['type'] = 'success';
                if($id){
                    $_GET['title'] = 'Atualização';
                    $_GET['message'] = 'Coluna L1-C1 atualizado com sucesso!';
                }else{
                    $_GET['title'] = 'Cadastro';
                    $_GET['message'] = 'Coluna L1-C1 cadastrado com sucesso!';
                }
            }
            else
            {
                ...
                if($id){
                    $_GET['title'] = 'Atualização';
                    $_GET['message'] = 'Ocorreu um erro ao atualizar o coluna L1-C1!';
                }else{
                    $_GET['title'] = 'Cadastro';
                    $_GET['message'] = 'Ocorreu um erro ao cadastrar o coluna L1-C1!';
                }
            }
            $this->index();
            $url = base_url(array('home', 'l1-c1'));
